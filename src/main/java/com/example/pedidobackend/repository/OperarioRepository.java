...
public interface OperarioRepository extends JpaRepository<Operario, Long> {
        ...
        @Query(value = "SELECT CASE WHEN COUNT(op.id_operario) > 0 THEN 'DNI_DUPLICADO' " +
                        "WHEN COUNT(op2.id_operario) > 0 THEN 'EMAIL_DUPLICADO' " +
                        "ELSE 'OK' END as resultado " +
                        "FROM (SELECT 1) dummy " +
                        "LEFT JOIN operario op ON op.numero_documento = :numeroDocumento AND op.estado = true " +
                        "LEFT JOIN operario op2 ON op2.correo_electronico = :correoElectronico AND op2.estado = true", nativeQuery = true)
        String validarOperarioUnico(@Param("numeroDocumento") String numeroDocumento,
